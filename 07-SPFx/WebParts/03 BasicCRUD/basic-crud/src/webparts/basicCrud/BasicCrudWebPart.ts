...
export interface IBasicCrudWebPartProps {
  description: string;
}
...
> {
  ...
  private generateListItemsHtml(): void {
    ...
    this.getListItems().then((data: ListItem[]) => {
      ...
      document.getElementById("theader").style.display =
        count === 0 ? "none" : "";
      ...
      for (let i: number = 0; i < count; i++) {
        ...
        const buttons = rootContainer.querySelectorAll(
  ...
  private addNewListItem(): void {
    ...
    const buttons = rootContainer
      .querySelectorAll("tr")
      [rootContainer.querySelectorAll("tr").length - 1].querySelectorAll(
        "button"
      );
  ...
  private getListItems(): Promise<ListItem[]> {
    return this.context.spHttpClient
      .get(
        this.context.pageContext["web"]["absoluteUrl"] +
          `/_api/web/lists/GetByTitle('${this._listName}')/items?$select=Id,Title`,
        SPHttpClient.configurations.v1
      )
      .then(
  ...
  private saveListItem(ContainerNode: Element, Id: string): void {
    ...
    this._getListItemEntityTypeFullName().then(
      (listItemEntityTypeFullName: string) => {
        ...
        if (Id === "0") {
          ...
          this.context.spHttpClient
            ...
            .then(
              (response: SPHttpClientResponse): Promise<ListItem> => {
                return response.json();
              }
            )
            .then(
              (item: ListItem): void => {
                ContainerNode.querySelectorAll("button")[0].textContent =
                  "Update";
                ContainerNode.querySelectorAll(
                  "button"
                )[0].parentElement.parentElement.setAttribute(
                  "data-id",
                  item.Id.toString()
                );
                this._showSuccess(
